...
import ru.ynovka.myShore.games.tag.states.TagState
...
class TagGame(val lobby: Lobby) : Game {
    ...
    val players: MutableMap<UUID, PlayerRoles> =
        lobby.members.associateWith { PlayerRoles.UNDEFINED }.toMutableMap()
    ...
    var totalTime: Int = 60
        set(value) {
            remainingTime += value - field   // сохраняем дельту
            field = value
        }
    ...
    private var stateImpl: TagState = stateOf(state)
    ...
    override fun join(player: Player) {
        players[player.uniqueId] = PlayerRoles.UNDEFINED
    ...
    private fun stateOf(state: TagGameStates): TagState = when (state) {
...
enum class TagGameStates {
    ...
    IN_PROGRESS,          // 60–150 сек — геймплей
...
fun TagGameMap.teleport(player: Player, game: TagGame, onComplete: () -> Unit = {}) {
    ...
    val destination = when (role) {
        PlayerRoles.UNDEFINED,
        PlayerRoles.VICTIM -> victimSpawns
            .shuffled()
            .firstOrNull { it.toLocation().getNearbyPlayers(1.0).isEmpty() }
            ?.toLocation()
            ?: victimSpawns.random().toLocation()

        PlayerRoles.HUNTER -> hunterSpawn.toLocation()

        PlayerRoles.SPECTATOR,
        PlayerRoles.SPECTATOR_VICTIM -> {
...
fun TagGame.hasVictims(): Boolean = players.values.any { it == PlayerRoles.VICTIM }
fun TagGame.hasHunter(): Boolean = players.values.any { it == PlayerRoles.HUNTER }
